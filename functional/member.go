...
var (
	binDir             = ".versions"
	etcdStarterBinPath = os.Getenv("ETCD_STARTER_BIN")
)
...
func init() {
	...
	if err != nil {
		log.Fatalf("abs on %s error: %v", binDir, err)
	}
}
...
func NewProcWithDefaultFlags() *Proc {
	var args []string
	...
	p := &Proc{
		Cmd:     exec.Command(etcdStarterBinPath, args...),
		Name:    "default",
		DataDir: dir,
	...
	p.Env = append(p.Env,
		"ETCD_INTERNAL_BINARY_DIR="+binDir,
	)
	return p
}

func NewProcWithName(name string) *Proc {
	p := NewProcWithDefaultFlags()
	p.SetName(name)
	return p
}

func NewProcWithV1Flags() *Proc {
	p := NewProcWithDefaultFlags()
	p.SetV1PeerAddr("127.0.0.1:7001")
	return p
}

func NewProcWithV2Flags() *Proc {
	p := NewProcWithDefaultFlags()
	p.SetV2PeerURL("http://127.0.0.1:7001")
	return p
}

func (p *Proc) UseBinary(name string) {
	p.Cmd = exec.Command(name, p.Args[1:]...)
	p.Env = append(p.Env,
		"ETCD_INTERNAL_BINARY_DIR="+binDir,
	)
}
...
func (p *Proc) Start() error {
	log.Printf("start using args %+v", p.Args)
...
func NewProcInProcGroupWithV1Flags(num int, idx int) *Proc {
	pg := NewProcGroupWithV1Flags(num)
	return pg[idx]
}

func NewProcGroupWithV1Flags(num int) ProcGroup {
	pg := make([]*Proc, num)
	for i := 0; i < num; i++ {
		pg[i] = NewProcWithDefaultFlags()
...
func NewProcGroupViaDiscoveryWithV1Flags(num int, url string) ProcGroup {
	pg := make([]*Proc, num)
	for i := range pg {
		pg[i] = NewProcWithDefaultFlags()
		pg[i].SetName(fmt.Sprintf("etcd%d", i))
...
func (pg ProcGroup) UseBinary(name string) {
	for i := range pg {
		pg[i].UseBinary(name)
	}
}
