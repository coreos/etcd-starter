...
func TestStartNewMember(t *testing.T) {
	tests := []*Proc{
		NewProcWithDefaultFlags(),
		NewProcWithName("etcd"),
		NewProcWithV1Flags(),
		NewProcWithV2Flags(),
	}
...
func TestUpgradeV1Cluster(t *testing.T) {
	...
	pg := NewProcGroupWithV1Flags(3)
	pg.UseBinary(v1BinPath)
...
func TestJoinV1ClusterViaDiscovery(t *testing.T) {
	dp := NewProcWithDefaultFlags()
	dp.UseBinary(v1BinPath)
	...
	pg := NewProcGroupViaDiscoveryWithV1Flags(1, durl)
	pg.UseBinary(v1BinPath)
	if err := pg.Start(); err != nil {
		t.Fatalf("Start error: %v", err)
	}
	pg.Stop()
	npg := NewProcGroupViaDiscoveryWithV1Flags(3, durl)
	npg[0].SetDataDir(pg[0].DataDir)
	if err := npg.Start(); err != nil {
		t.Fatalf("Start error: %v", err)
	}
...
func TestUpgradeV1Standby(t *testing.T) {
	...
	p := NewProcInProcGroupWithV1Flags(4, 3)
	if err := p.Start(); err != nil {
		t.Fatalf("Start error: %v", err)
	}
	...
	np := NewProcInProcGroupWithV1Flags(4, 3)
